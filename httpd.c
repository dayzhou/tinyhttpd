...
void* accept_request(void* clt)
{
 ...
 // 如果是 POST 方法就将 cgi 标志变量置为1(true)
 // 也就意味着实现了 cgi 功能
 ...
 if (strcasecmp(method, "GET") == 0)
 {
  ...
  if (*query_string == '?')
  {
   // 如果是 ? 的话，证明这个请求需要调用 cgi，将 cgi 标志变量置为1(true)
   cgi = 1;
 ...
 // 将前面分隔两份的前面那份字符串，拼在字符串htdocs后并输出存储到数组 path 中
 // 相当于现在 path 中存储着一个字符串
 sprintf(path, "htdocs%s", url);
 
 // 如果 path 数组中的这个字符串的最后一个字符是以字符 / 结尾的话
 // 就拼接上一个"index.html"的字符串表示首页的意思
 if (path[strlen(path) - 1] == '/')
  strcat(path, "index.html"); // 注：首页不一定就是它
 ...
 else
 {
  ...
  if ((st.st_mode & S_IXUSR) ||       
      (st.st_mode & S_IXGRP) ||
      (st.st_mode & S_IXOTH)    )
   // 如果这个文件是一个可执行文件，不论是属于用户/组/其他这三者类型的，就将 cgi 标志变量置为1
   cgi = 1;
                 ...
                 const char *method, const char *query_string)
{
 ...
 // strcmp 是根据 ascii 码值来比较的，字符码值相等继续否则停止
 buf[0] = 'A'; buf[1] = '\0';
 // 如果是 http 请求 GET 方法的话读取并忽略请求剩下的内容
 if (strcasecmp(method, "GET") == 0)
  while ((numchars > 0) && strcmp("\n", buf))  /* read & discard headers */
   numchars = get_line(client, buf, sizeof(buf));
 else    /* POST */
 ...
 if (pid == 0)  /* child: CGI script */
 {
  ...
  // dup2()包含在<unistd.h>中，参读《TLPI》P97
  ...
  // 父进程关闭 cgi_output 管道的写端和 cgi_input 管道的读端
  close(cgi_output[1]);
  close(cgi_input[0]);
  
  // 如果是 POST 方法就继续读 body 的内容，并写到 cgi_input 管道里让子进程去读
  if (strcasecmp(method, "POST") == 0)
...
int startup(u_short *port)
{
 ...
 if (*port == 0)  /* if dynamically allocating a port */
 {
  ...
  // name.sin_port 随机端口，使用ntohs将16位网络字符序转换为主机字符序
...
/**********************************************************************/
/* 未实现 GET 和 POST 协议时 response 的内容 */
...
int main(void)
{
 ...
 while (1)
 {
  ...
  // 创建线程，accept_request是线程处理函数，client_sock是参数
